...
class RBAC {
    ...
    static public function authenticate($map,$model='') {
        if(empty($model)) $model =  USER_AUTH_MODEL;
        //使用给定的Map进行认证
    ...
    static function saveAccessList($authId=null) {
        if(null===$authId)   $authId = $_SESSION[USER_AUTH_KEY];
	...
	static public function checkLogin() {
        ...
        if(RBAC::checkAccess()) {
            //检查认证识别号
            if(!$_SESSION[USER_AUTH_KEY]) {
                if(GUEST_AUTH_ON) {
                    // 开启游客授权访问
                    if(!isset($_SESSION['_ACCESS_LIST']))
                        // 保存游客权限
                        RBAC::saveAccessList(GUEST_AUTH_ID);
                }else{
                    // 禁止游客访问跳转到认证网关
        ...
        if(RBAC::checkAccess()) {
            ...
            if(empty($_SESSION[ADMIN_AUTH_KEY])) {
                if(USER_AUTH_TYPE==2) {
                    ...
                    //通过数据库进行访问检查
                    $accessList = RBAC::getAccessList($_SESSION[USER_AUTH_KEY]);
                }else {
    ...
    static public function getAccessList($authId) {
        // Db方式权限数据
        $db     =   Db::getInstance(RBAC_DB_DSN);
        $table = array('role'=>RBAC_ROLE_TABLE,'user'=>RBAC_USER_TABLE,'access'=>RBAC_ACCESS_TABLE,'node'=>RBAC_NODE_TABLE);
	...
	static public function getModuleAccessList($authId,$module) {
        // Db方式
        $db     =   Db::getInstance(RBAC_DB_DSN);
        $table = array('role'=>RBAC_ROLE_TABLE,'user'=>RBAC_USER_TABLE,'access'=>RBAC_ACCESS_TABLE);
