...
abstract class RestAction {
   ...
   /**
    ...
    public function __construct() {
        ...
        }elseif(false === stripos(REST_CONTENT_TYPE_LIST,__EXT__)) {
            // 资源类型非法 则用默认资源类型访问
            $this->_type   =  REST_DEFAULT_TYPE;
        }else{
            // 检测实际资源类型
        ...
        if(false === stripos(REST_METHOD_LIST,$method)) {
            // 请求方式非法 则用默认请求方法
            $method = REST_DEFAULT_METHOD;
        }
        $this->_method = $method;
        // 允许输出的资源类型
        $this->_types  = REST_OUTPUT_TYPE;
   ...
   /**
    ...
    protected function isAjax() {
        ...
        if(!empty($_POST[VAR_AJAX_SUBMIT]) || !empty($_GET[VAR_AJAX_SUBMIT]))
            // 判断Ajax方式提交
            return true;
        return false;
    }
    ...
    public function __call($method,$args) {
        if( 0 === strcasecmp($method,ACTION_NAME)) {
            if(method_exists($this,$method.'_'.$this->_method.'_'.$this->_type)) { // RESTFul方法支持
                ...
                $this->$fun();
            }elseif($this->_method == REST_DEFAULT_METHOD && method_exists($this,$method.'_'.$this->_type) ){
                $fun  =  $method.'_'.$this->_type;
                $this->$fun();
            }elseif($this->_type == REST_DEFAULT_TYPE && method_exists($this,$method.'_'.$this->_method) ){
                $fun  =  $method.'_'.$this->_method;
                $this->$fun();
            }elseif(method_exists($this,'_empty')) {
                // 如果定义了_empty操作 则调用
                $this->_empty($method,$args);
            }elseif(file_exists_case(TMPL_FILE_NAME)){
            ...
            if(isset($input[$args[0]])) { // 取值操作
                $data	 =	 $input[$args[0]];
                $fun  =  $args[1]?$args[1]:DEFAULT_FILTER;
                $data	 =	 $fun($data); // 参数过滤
            }else{ // 变量默认值
