...
class UpgradeNoticeBehavior extends Behavior {
    ...
    public function run(&$params) {
        if (UPGRADE_NOTICE_ON && (!S('think_upgrade_interval') || UPGRADE_NOTICE_DEBUG)) {
            if(IS_SAE && UPGRADE_NOTICE_QUEUE && !isset($_POST['think_upgrade_queque'])){
                $queue=new SaeTaskQueue(UPGRADE_NOTICE_QUEUE);
            ...
            $akey = UPGRADE_NOTICE_AKEY;
            $skey = UPGRADE_NOTICE_SKEY;
            ...
            $current_version = UPGRADE_CURRENT_VERSION;
            //读取接口
            ...
            S('think_upgrade_interval', true, UPGRADE_NOTICE_CHECK_INTERVAL);
        }
    }
