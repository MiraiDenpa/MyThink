...
class SessionDb {
   ...
   protected $hander  = array(); 
    ...
    public function open($savePath, $sessName) { 
       $this->lifeTime = SESSION_EXPIRE?SESSION_EXPIRE:ini_get('session.gc_maxlifetime');
       $this->sessionTable  =   SESSION_TABLE?SESSION_TABLE:C("DB_PREFIX")."session";
       //分布式数据库
       $host = explode(',',DB_HOST);
       $port = explode(',',DB_PORT);
       $name = explode(',',DB_NAME);
       $user = explode(',',DB_USER);
       $pwd  = explode(',',DB_PWD);
       if(1 == DB_DEPLOY_TYPE){
           //读写分离
           if(DB_RW_SEPARATE){
               $w = floor(mt_rand(0,DB_MASTER_NUM-1));
               if(is_numeric(DB_SLAVE_NO)){//指定服务器读
                   $r = DB_SLAVE_NO;
               }else{
                   $r = floor(mt_rand(DB_MASTER_NUM,count($host)-1));
               }
               //主数据库链接
