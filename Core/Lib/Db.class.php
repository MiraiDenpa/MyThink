...
class Db{
			...
			if(is_string($val[0])){
				if(preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT)$/i', $val[0])){ // 比较运算
					...
					if(is_array($val[1])){
						...
						$whereStr .=
								$key . ' ' . $this->comparison[strtolower($val[0])] . ' ' . $this->parseValue($val[1]);
					}
	...
	public function update($data, $options){
		...
		$sql         = 'UPDATE ' . $this->parseTable($options['table']) . $this->parseSet($data) .
					   ...
					   $this->parseLock(isset($options['lock'])? $options['lock'] : false) . $this->parseComment(!empty($options['comment'])? $options['comment'] : '');
	...
	public function select($options = array()){
		...
		if($cache && false !== $result){ // 查询缓存写入
			SAS($cache['cas'], $key, $result, $cache['expire']);
		}
