...
class ReadHtmlCacheBehavior extends Behavior {
        ...
        // 分析当前的静态规则
         ...
         if(!empty($htmls)) {
            ...
            if(!empty($html)) {
                ...
                $cacheTime = isset($html[1])?$html[1]:HTML_CACHE_TIME; // 缓存有效期
                // 当前缓存文件
                define('HTML_FILE_NAME',HTML_PATH . $rule.HTML_FILE_SUFFIX);
                return $cacheTime;
            }
        }
        // 无需缓存
    ...
    static public function checkHTMLCache($cacheFile='',$cacheTime='') {
        if(!is_file($cacheFile)){
            return false;
        }elseif (filemtime(TEMPLATE_NAME) > filemtime($cacheFile)) {
            // 模板文件如果更新静态文件需要更新
            return false;
