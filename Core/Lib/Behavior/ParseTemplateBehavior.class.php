...
class ParseTemplateBehavior extends Behavior {
    ...
    public function run(&$_data){
        $engine             =   strtolower(TMPL_ENGINE_TYPE);
        $_content           =   empty($_data['content'])?$_data['file']:$_data['content'];
        $_data['prefix']    =   !empty($_data['prefix'])?$_data['prefix']:TMPL_CACHE_PREFIX;
        if('think'==$engine){ // 采用Think模板引擎
            if((!empty($_data['content']) && $this->checkContentCache($_data['content'],$_data['prefix'])) 
                ...
                //载入模版缓存文件
                include CACHE_PATH.$_data['prefix'].md5($_content).TMPL_CACHFILE_SUFFIX;
            }else{
    ...
    protected function checkCache($tmplTemplateFile,$prefix='') {
        ...
        // 开启布局模板
        if(LAYOUT_ON) {
            $layoutFile  =  THEME_PATH.LAYOUT_NAME.TMPL_TEMPLATE_SUFFIX;
    ...
    protected function checkContentCache($tmplContent,$prefix='') {
        if(is_file(CACHE_PATH.$prefix.md5($tmplContent).TMPL_CACHFILE_SUFFIX)){
            return true;
        }else{
            return false;
        }
