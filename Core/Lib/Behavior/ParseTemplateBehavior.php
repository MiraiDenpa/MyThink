...
class ParseTemplateBehavior extends Behavior{
	...
	public function run(&$_data){
		...
		if((!empty($_data['content']) && $this->checkContentCache($_data['content'], $_data['prefix'])) ||
		   $this->checkCache($_data['file'], $_data['prefix'])
			...
			//载入模版缓存文件
			include CACHE_PATH . $_data['prefix'] . md5($_content) . '.html';
		} else{
	...
	protected function checkCache($tmplTemplateFile, $prefix = ''){
		$tmplCacheFile = CACHE_PATH . $prefix . md5($tmplTemplateFile).'.html';
	...
	protected function checkContentCache($tmplContent, $prefix = ''){
		if(is_file(CACHE_PATH . $prefix . md5($tmplContent) . '.html')){
			return true;
		} else{
			return false;
		}
