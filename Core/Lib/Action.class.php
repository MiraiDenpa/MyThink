...
abstract class Action {
   ...
   /**
    ...
    public function __call($method,$args) {
        if( 0 === strcasecmp($method,ACTION_NAME.ACTION_SUFFIX)) {
            ...
            if(!isset($args[0])) { // 获取全局变量
                ...
                $data       =	$input[$args[0]];
                $filters    =   isset($args[1])?$args[1]:DEFAULT_FILTER;
    ...
    private function dispatchJump($message,$status=1,$jumpUrl='',$ajax=false) {
        ...
        if($status) { //发送成功信息
            ...
            $this->display(TMPL_ACTION_SUCCESS);
        }else{
            ...
            $this->display(TMPL_ACTION_ERROR);
            // 中止执行  避免出错后继续执行
            exit ;
        }
    }
