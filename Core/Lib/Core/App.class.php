...
class App {
    ...
    static public function init() {
        ...
        C('TMPL_EXCEPTION_FILE',realpath(TMPL_EXCEPTION_FILE));
        return ;
    }
    ...
    static public function exec() {
        if(!preg_match('/^[A-Za-z](\w)*$/',MODULE_NAME)){ // 安全检测
            ...
            //创建Action控制器实例
            $group   =  defined('GROUP_NAME') && APP_GROUP_MODE==0 ? GROUP_NAME.'/' : '';
        ...
        try{
            ...
            if($method->isPublic()) {
                ...
                // URL参数绑定检测
                if(URL_PARAMS_BIND && $method->getNumberOfParameters()>0){
