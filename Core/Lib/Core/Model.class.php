...
class Model {
    ...
    // 链操作方法列表
    protected $methods          =   array('table','order','alias','having','group','lock','distinct','auto','filter','validate','result','bind','token','page');
	// 回调函数名称列表
	protected $callback = array('initialize','create',
								'after_db',
								// 前置过滤
								'options_filter','data_filter',
    ...
    public function flush() {
        ...
        if(DB_FIELD_VERSION) $this->fields['_version'] =   DB_FIELD_VERSION;
    ...
    protected function _parseOptions($options=array()) {
        ...
        // 表达式过滤
		$this->doCallback('options_filter', $data=array(), $options);
        return $options;
    }
    ...
    public function autoCheckToken($data) {
        ...
        if(TOKEN_ON){
            ...
            // 开启TOKEN重置
            if(TOKEN_RESET) unset($_SESSION[$name][$key]);
            return false;
        }
        return true;
    }
	...
	public function page($page_var_name = 'p'){
		...
		if($not_init){
			$this->register_callback('options_filter',function(&$data, &$options){
				if(!isset($options['pager']) || !$options['pager']) return;
	...
	public function register_callback($type, $cb){
		if(!in_array($type,$this->callback)) throw_exception('Model： 注册不存在的事件`'.$type.'`。');
		if(is_string($cb)) $cb = array($this, $cb);
	...
	private function doCallback($type, &$data, &$opt ){
		if(!isset($this->callback[$type])) return true;
