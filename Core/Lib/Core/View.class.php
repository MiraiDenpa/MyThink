...
class View {
    ...
    private function render($content,$charset='',$contentType=''){
        ...
        if(empty($contentType)) $contentType = TMPL_CONTENT_TYPE;
        // 网页字符编码
    ...
    public function parseTemplate($template='') {
        ...
        if(is_file($template)) {
            ...
            if(1==APP_GROUP_MODE){ // 独立分组模式
                ...
                define('APP_TMPL_PATH',__ROOT__.'/'.$app_name.APP_GROUP_PATH.'/'.$group.basename(TMPL_PATH).'/'.$theme);
            }else{ 
        ...
        return THEME_PATH.$template.TMPL_TEMPLATE_SUFFIX;
    }
    ...
    private function getTemplateTheme() {
        if($this->theme) { // 指定模板主题
            ...
            if(TMPL_DETECT_THEME) {// 自动侦测模板主题
                ...
                if(!in_array($theme,explode(',',THEME_LIST))){
                    $theme =  DEFAULT_THEME;
                }
