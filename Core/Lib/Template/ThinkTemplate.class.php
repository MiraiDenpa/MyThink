...
class  ThinkTemplate {
    ...
    public function __construct(){
        ...
        $this->config['template_suffix']    =   TMPL_TEMPLATE_SUFFIX;
        $this->config['cache_suffix']       =   TMPL_CACHFILE_SUFFIX;
        $this->config['tmpl_cache']         =   TMPL_CACHE_ON;
        $this->config['cache_time']         =   TMPL_CACHE_TIME;
        $this->config['taglib_begin']       =   $this->stripPreg(TAGLIB_BEGIN);
        $this->config['taglib_end']         =   $this->stripPreg(TAGLIB_END);
        $this->config['tmpl_begin']         =   $this->stripPreg(TMPL_L_DELIM);
        $this->config['tmpl_end']           =   $this->stripPreg(TMPL_R_DELIM);
        $this->config['default_tmpl']       =   TEMPLATE_NAME;
    ...
    public function loadTemplate ($tmplTemplateFile,$prefix='') {
        ...
        if(LAYOUT_ON) {
            if(false !== strpos($tmplContent,'{__NOLAYOUT__}')) { // 可以单独定义不使用布局
                ...
                $layoutFile  =  THEME_PATH.LAYOUT_NAME.$this->config['template_suffix'];
    ...
    protected function parsePhp($content) {
        ...
        // PHP语法检查
        if(TMPL_DENY_PHP && false !== strpos($content,'<?php')) {
    ...
    public function parseTagLib($tagLib,&$content,$hide=false) {
        ...
        $tLib       =   ThinkInstance::get($className);
    ...
    public function parseXmlTag($tagLib,$tag,$attr,$content) {
        ...
        $tLib       =  ThinkInstance::get('TagLib'.ucwords(strtolower($tagLib)));
        $parse      = '_'.$tag;
